...
import { ArrowLeft, Building2, MapPin, Calendar, DollarSign, TrendingUp, Users, BarChart3, FileText, Target, Loader2, AlertTriangle, Star, Globe, Zap, Brain, Award, Rocket, Upload, RefreshCw } from 'lucide-react';
...
const CompanyDetailPage = () => {
  ...
  const fetchCompanyData = async (companyId: string) => {
    try {
      ...
      // Process results
      if (financialData.status === 'fulfilled' && !financialData.value.error) {
        setFinancialModel(financialData.value.data?.[0] || null);
      }
      if (pitchData.status === 'fulfilled' && !pitchData.value.error) {
        setPitchDeck(pitchData.value.data?.[0] || null);
      }
      if (gtmData.status === 'fulfilled' && !gtmData.value.error) {
        setGoToMarket(gtmData.value.data?.[0] || null);
      }
      if (vcData.status === 'fulfilled' && !vcData.value.error) {
        setVcFitReport(vcData.value.data?.[0] || null);
      }
      if (foundersData.status === 'fulfilled' && !foundersData.value.error) {
        setFounders(foundersData.value.data || []);
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="text-center relative z-10 max-w-md">
          ...
          <p className="text-sm text-gray-500 mb-6">Company ID: {id}</p>
              ...
              <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)} className="w-full">
                <div className="bg-white rounded-xl shadow-lg">
                  <div className="border-b border-gray-200">
                    <TabsList className="grid w-full grid-cols-3 lg:grid-cols-6 p-1 bg-transparent">
                      {tabs.map((tab) => {
                        const Icon = tab.icon;
                        return (
                          <TabsTrigger
                            key={tab.id}
                            value={tab.id}
                            className="flex items-center space-x-2 py-3 px-4 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-600 rounded-lg transition-all duration-200"
                          >
                        ...
                        <TabsContent value="overview" className="mt-0">
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            {founders.length > 0 ? (
                              ...
                              <GlowingCard glowColor="gray" intensity="low">
                                <div className="bg-white rounded-xl shadow-lg p-6 text-center">
                                  <Users className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                                  <h3 className="text-lg font-medium text-gray-900 mb-2">No Team Data</h3>
                                  <p className="text-gray-600 mb-4">Founder information not available</p>
                                  <MorphingButton
                                    variant="outline"
                            ...
                            <div className="bg-white rounded-xl shadow-lg p-6">
                              ...
                              {financialModel ? (
                                    ...
                                    <div className="bg-purple-50 rounded-lg p-4 cursor-help hover:bg-purple-100 transition-colors">
                                      <div className="text-2xl font-bold text-purple-600">
                                        {financialModel.runway_months !== null && financialModel.runway_months !== undefined 
                                          ? `${Math.round(financialModel.runway_months)} mo`
                                          : 'N/A'}
                                      </div>
                                ...
                                <div className="text-center py-8">
                                  ...
                                  <p className="text-gray-600 mb-4">No financial model data available</p>
                                  <MorphingButton
                                    variant="outline"
